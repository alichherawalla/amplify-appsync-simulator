...
describe('runQueryAndMutation', () => {
    const schemaDoc = graphql_1.parse(`
    type Query {
      getName: String
    }
    type Mutation {
  ...
  `);
    ...
    beforeEach(() => {
        ...
        schema = graphql_tools_1.makeExecutableSchema({ typeDefs: schemaDoc, resolvers });
        executionContext = {
    ...
    `);
        ...
        await expect(query_and_mutation_1.runQueryOrMutation(schema, doc, variables, undefined, executionContext)).resolves.toEqual({ data: { getName: name } });
    ...
    `);
        ...
        getNameResolver.mockReturnValue(name);
        await expect(query_and_mutation_1.runQueryOrMutation(schema, doc, variables, undefined, executionContext)).resolves.toEqual({ data: { setName: name } });
    ...
    `);
        const variables = {};
        getNameResolver.mockReturnValue(name);
        await expect(query_and_mutation_1.runQueryOrMutation(schema, doc, variables, undefined, executionContext)).resolves.toEqual({
            data: {
                onSetName: null,
            },
        });
    ...
    `);
        const variables = { var1: 'val1' };
        getNameResolver.mockReturnValue(name);
        await expect(query_and_mutation_1.runQueryOrMutation(schema, doc, variables, 'getName', executionContext)).resolves.toEqual({ data: { getName: name } });
        expect(getNameResolver).toHaveBeenCalled();
        expect(getNameResolver.mock.calls[0][2]).toEqual(executionContext);
    });
    ...
    `);
        const variables = { var1: 'val1' };
        getNameResolver.mockReturnValue(name);
        await expect(query_and_mutation_1.runQueryOrMutation(schema, doc, variables, 'getName', executionContext)).resolves.toEqual({
            data: null,
