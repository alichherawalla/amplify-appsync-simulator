...
describe('getOperationType', () => {
    ...
    `);
        expect(helpers_1.getOperationType(doc)).toEqual('subscription');
    });
    ...
    `);
        expect(helpers_1.getOperationType(doc)).toEqual('mutation');
    });
    ...
    `);
        expect(helpers_1.getOperationType(doc)).toEqual('query');
    });
    ...
    `);
        expect(helpers_1.getOperationType(doc, 'listUsers')).toEqual('query');
    });
    ...
    `);
        expect(() => helpers_1.getOperationType(doc)).toThrowError('Could not get operation');
    });
