...
describe('decodeHeaderFromQueryParam', () => {
    it('should decode header query param from the the url', () => {
        ...
        expect(decode_header_1.decodeHeaderFromQueryParam(url)).toEqual(header);
        expect(decode_header_1.decodeHeaderFromQueryParam(url, paramName)).toEqual(header);
    });
    it('should return an empty object when the parameter is missing in the query', () => {
        const url = `/graphql`;
        const paramName = 'header';
        expect(decode_header_1.decodeHeaderFromQueryParam(url)).toEqual({});
        expect(decode_header_1.decodeHeaderFromQueryParam(url, paramName)).toEqual({});
    });
});
