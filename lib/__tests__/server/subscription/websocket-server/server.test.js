...
describe('WebsocketSubscriptionServer', () => {
    ...
    beforeEach(async () => {
        ...
        keepAlive = 100;
        server = new server_1.WebsocketSubscriptionServer({
            onSubscribeHandler,
            onConnectHandler,
            connectionTimeoutDuration,
            keepAlive,
        }, { server: httpServer, path: SUBSCRIPTION_PATH });
        serverPort = await portfinder_1.default.getPortPromise({
            startPort: 20001,
            stopPort: 66666,
        });
    ...
    describe('Connect', () => {
        it('should close connection when the protocol is not graphql-ws', done => {
            const client = new ws_1.default(`ws://localhost:${serverPort}${SUBSCRIPTION_PATH}`, 'something');
        ...
        it('should accept connection when the protocol is graphql-ws', async () => {
            const client = new ws_1.default(`ws://localhost:${serverPort}${SUBSCRIPTION_PATH}`, 'graphql-ws');
        ...
        it('should call onConnectionHandler with header', async () => {
            ...
            const client = new ws_1.default(`ws://localhost:${serverPort}${SUBSCRIPTION_PATH}?${query.toString()}`, 'graphql-ws');
            await waitForConnection(client);
            client.close();
            expect(onConnectHandler).toHaveBeenCalled();
    ...
    describe('Connection init', () => {
        ...
        beforeEach(async () => {
            jest.useFakeTimers();
            const url = new url_1.URL(`ws://localhost:${serverPort}${SUBSCRIPTION_PATH}`).toString();
    ...
    `;
        ...
        it('should send MESSAGE_TYPES.GQL_START_ACK on when a subscription is stated', async () => {
            ...
            expect(onSubscribeHandler).toHaveBeenCalledWith(graphql_1.parse(query), variables, headers, connectionContext.request);
        ...
        it('should add multiple subscriptions in same connection', async () => {
            ...
            expect(onSubscribeHandler).toHaveBeenLastCalledWith(graphql_1.parse(query), variables, headers, connectionContext.request);
